...
(
    Gpt_PredefTimerType enPredefTimer,
    uint32 coreID
);
 ...
 *                 In case (Gpt_PredefTimerType != enPredefTimer)
 *                 In case pointer to the GPT channel predeftimer configuration == NULL_PTR
 *                 The function will report an error to DET or SERR, depending on the environment the driver is run in.
 *
 * @param[in]      enPredefTimer Parameter of PredefTimer functionality
 * @return         The validity of the function call
 * @returnValue    E_OK     The function call is valid
 * @returnValue    E_NOT_OK The function call is invalid
 *
...
)
{
    ...
    Gpt_ChannelType channel = (Gpt_ChannelType)enPredefTimer;

    if ((enPredefTimer != GPT_PREDEF_TIMER_1US_16BIT) && \
        (enPredefTimer != GPT_PREDEF_TIMER_1US_24BIT) && \
        (enPredefTimer != GPT_PREDEF_TIMER_1US_32BIT) && \
        (enPredefTimer != GPT_PREDEF_TIMER_100US_32BIT))
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
...
void Gpt_DeInit(void)
{
    ...
    if((Std_ReturnType)E_OK == Gpt_ValidateGlobalCall(GPT_DEINIT_ID, coreID))
    {
        ...
        for (ChannelIndex = 0U; (ChannelIndex < Gpt_pConfig[coreID]->channelCount); ChannelIndex++)
        {
            ...
            allChannelStatus = (channel < GPT_HW_CHANNEL_NUM) ? Gpt_ValidateChannelStatus(GPT_DEINIT_ID, channel, coreID) : (Std_ReturnType)E_NOT_OK;
...
{
    Std_ReturnType returnValue;
    Gpt_ChannelType channel = (Gpt_ChannelType)PredefTimer;
    uint32 coreID = (uint32)Gpt_GetCoreID();
