...
static uint16 MemAcc_GetAddressAreaIndex(const MemAcc_AddressAreaIdType AddressAreaId)
{
    ...
    {
        Index = MEMACC_ADDRESS_AREA_INDEX_INVALID;
    }
...
static uint32 MemAcc_ValidateAlignment(MemAcc_JobRuntimeInfoType *JobRequest)
{
    ...
    if ( (PhysicalSegmentSize == 0U) || \
         ((JobRequest->LogicAddress % PhysicalSegmentSize) != 0U) || \
         ((JobRequest->LengthOrigin % PhysicalSegmentSize) != 0U) \
       )
    {
...
static uint32 MemAcc_ValidateAddressAndLength(MemAcc_JobRuntimeInfoType *JobRequest)
{
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        /* Alignment checking */
        /* TODO: perform check for all SubAreas which span in this job, not only the first one */
...
/**
 * @brief    Checks if provided AddressAreaId is consistent with the configuration
 */
static uint32 MemAcc_ValidateAddressAreaId(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint32 ErrorId = MEMACC_E_OK;
 ...
 */
static uint32 MemAcc_ValidateParamJobCommon(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint32 ErrorId;
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        ErrorId = MemAcc_ValidateAddressAreaId(JobRequest);
    }
 ...
 */
static uint32 MemAcc_ValidateParamJobDataTransfer(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint32 ErrorId;

    /* Perform the common checking */
    ErrorId = MemAcc_ValidateParamJobCommon(JobRequest);
...
{
    MemAcc_JobRuntimeInfoType *JobCurrent = &MemAcc_pJobRuntimeInfo[JobRequest->AreaIndex];
 ...
 **/
static void MemAcc_JobEndNotify( MemAcc_JobRuntimeInfoType  *JobInfo )
{
...
static void MemAcc_JobRequestMemService( MemAcc_JobRuntimeInfoType *JobInfo )
{
    ...
    switch (JobInfo->JobType)
    {
        ...
        default:
            /* TODO: implement other jobs: HwSpecific */
            break;
    };
...
*/
static boolean MemAcc_CompareCheckData(MemAcc_DataType *SourcePtr, MemAcc_DataType *DestPtr, MemAcc_LengthType length)
{
    uint32 counter;
 ...
 **/
static void MemAcc_ScheduleMemDriver( MemAcc_JobRuntimeInfoType *JobInfo )
{
...
MemAcc_JobResultType MemAcc_GetJobResult(MemAcc_AddressAreaIdType AddressAreaId)
{
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        {
            /* Report the error */
            MemAcc_ReportDevError(MEMACC_GETJOBSTATUS_ID, MEMACC_E_PARAM_ADDRESS_AREA_ID);
        }
    }
    else
    {
...
)
{
    ...
    else
    {
        ...
        else
        {
            ...
            SubAreaInfo = &(MemAcc_pJobRuntimeInfo[AreaIndex].SubArea[SubAreaIndex]);
...
MemAcc_LengthType MemAcc_GetProcessedLength(MemAcc_AddressAreaIdType AddressAreaId)
{
    ...
    {
        /* Check if the provided AddressAreaId is consistent with the configuration */
        /* And get the index number of this address area */
        AreaIndex = MemAcc_GetAddressAreaIndex(AddressAreaId);
...
)
{
    ...
    uint32 ErrorId;
    MemAcc_MemApiType *pMemApis = NULL_PTR;
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        if(MemAcc_pConfigPtr->MemApiCount > HwId)
        {
            ...
            {
                /* Todo: Validate the Mem driver binary */
                pMemApis = (MemAcc_MemApiType *)HeaderAddress;
...
)
{
    ...
    ErrorId = MemAcc_ValidateParamJobDataTransfer(&JobRequest);
...
)
{
    ...
    MemAcc_InitJobRequest(&JobRequest, AddressAreaId, TargetAddress, (MemAcc_DataType *) ((uint32)SourceDataPtr), Length);
...
)
{
    ...
    MemAcc_InitJobRequest(&JobRequest, AddressAreaId, SourceAddress, (MemAcc_DataType *)((uint32)DataPtr), Length);
